...
@section('titulo','Unit 63 | Listening')
@section('conteudo')
<!-- WRAPPER ALL -->
...
<main>
    ...
    <div class="dashboard">
        <div id="unidade" class="box" data-apostila="apostila2" data-unidade="unidade63" data-etapa="listening">
            <h3 class="barlow">UNIT 63</h3>
            <h5 class="barlow">5 - LISTENING</h5>
            <span class="play-audio">( Aperte o play )</span>
            <audio controls="" controlslist="nodownload" class="suave">
                <source src="{{ asset('assets/audio/apostila2/UNIT 63/4. LISTENING/LISTENING.ogg') }}" type="audio/ogg">
            </audio>
            <div class="clear"></div>
...
<script>
    ...
    $("#unidade63listening414").submit(function(e) {
        e.preventDefault();
        $(this).find('button').prop('disabled', true);
        var respostas = '{';
        $('#unidade63listening414 input[type="text"]').each(function(index) {
            if (($('#unidade63listening414 input[type="text"]').length - 1) == index) {
                respostas += '"' + $(this).attr("name") + '":"' + $(this).val() + '"}';
            } else {
                respostas += '"' + $(this).attr("name") + '":"' + $(this).val() + '",';
            }
        });
        if ($('#unidade63listening414 input[name="resposta_id"').val() != 0) {
            atualizarAtividade($('#unidade63listening414'), respostas);
        } else {
            enviarAtividade($('#unidade63listening414'), respostas);
        }
    });
        ...
        request.done(function(response) {
            if (response == 0) {
                ...
                for (j = 0; j < respostas.length; j++) {
                    $('#unidade63listening' + atividade_id + ' input[name="' + chaves[j] + '"]').val(respostas[j]);
                    $('#unidade63listening' + atividade_id + ' input[name="' + chaves[j] + '"]').attr("value", respostas[j]);
                }
                $('#unidade63listening' + atividade_id + ' input[name="resposta_id"]').val(response[0].resposta_id);
                $('#unidade63listening' + atividade_id + ' input[name="resposta_id"]').attr("value", response[0].resposta_id);
            }
        });
    }

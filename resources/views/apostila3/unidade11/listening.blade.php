...
<!-- WRAPPER ALL -->
	...
	<main>
		...
		<div class="dashboard">
			<div id="unidade" class="box" data-apostila="apostila3" data-unidade="unidade11" data-etapa="listening">
    ...
    <script>
        ...
        $("#unidade11listening565").submit(function(e){
            e.preventDefault();
            $(this).find('button').prop('disabled', true);
            var respostas = '{';
            $('#unidade11listening565 input[type="text"]').each(function(index){
                if(($('#unidade11listening565 input[type="text"]').length - 1) == index){
                    respostas += '"'+$(this).attr("name")+'":"'+$(this).val()+'"}';
                }else{
                    respostas += '"'+$(this).attr("name")+'":"'+$(this).val()+'",';
                }
            });
            if($('#unidade11listening565 input[name="resposta_id"').val() != 0){
                atualizarAtividade($('#unidade11listening565'), respostas);
            }else{
                enviarAtividade($('#unidade11listening565'), respostas);
            }
        });
            ...
            request.done(function(response){
                if(response == 0){
                    ...
                    for(j = 0; j < respostas.length; j++){
                        $('#unidade11listening'+atividade_id+' textarea[name="'+chaves[j]+'"]').val(respostas[j]);
                        $('#unidade11listening'+atividade_id+' textarea[name="'+chaves[j]+'"]').attr("value", respostas[j]);
